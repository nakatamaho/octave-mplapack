...
**Corresponding example:** `examples/tiered/neig-tier-a/frank1.m`

**Original tier/source:** The reflected formula and provenance are in `docs/codex/neigt/CASES.md` Section 6.

## Question

The explicitly reflected Frank orientation. This is a one-case view of the repository's A tier; the complete profile remains the regression authority.

## Matrix or problem

A2 reflected orientation `F1=R*F0'*R` of the manifest Frank matrix.
...
Reflection changes eigenvectors and orientation while retaining the intended spectral family.
...
`mp_neig_tiers` with `tier="A"`, `case_id="FRANK1"`, and the public `eig` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

Inspect the actual input hash, matched spectrum, left/right residuals, and balance effect.
...
Do not silently identify F1 with F0 just because both are called Frank matrices.
