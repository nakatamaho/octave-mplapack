...
%
% Question: The explicitly symmetric coordinate representation of the Toeplitz control.
% Steps:
...
% Matching documentation: docs/examples/tiered/neig-tier-s/toeplitz_sym.md
...
result = mp_neig_tiers ("smoke", struct ("tier", "S", ...
                                           "case_id", "TOEPLITZ_SYM", ...
                                           "plot", false));
assert (result.ok && result.coverage.case_count == 1);
fprintf ("TOEPLITZ_SYM: PASS; selected cases=%d; measured rows=%d\n", ...
         result.coverage.case_count, result.coverage.measured_eig_rows);
fprintf ("See docs/examples/tiered/neig-tier-s/toeplitz_sym.md for interpretation.\n");
