% Two Jordan clusters.
%
% Question: Two nearby defective clusters and their merged-subspace warning.
% Steps:
...
% Matching documentation: docs/examples/tiered/neig-tier-s/sim_two_jordan.md
...
result = mp_neig_tiers ("smoke", struct ("tier", "S", ...
                                           "case_id", "SIM_TWO_JORDAN", ...
                                           "plot", false));
assert (result.ok && result.coverage.case_count == 1);
fprintf ("SIM_TWO_JORDAN: PASS; selected cases=%d; measured rows=%d\n", ...
         result.coverage.case_count, result.coverage.measured_eig_rows);
fprintf ("See docs/examples/tiered/neig-tier-s/sim_two_jordan.md for interpretation.\n");
