# Near-rank-deficient Hadamard spectrum

**Corresponding example:** `examples/tiered/svd-tier-a/hadamard_rank5.m`

**Original tier/source:** The near-rank-deficient mode is a distinct A5 manifest fixture.

## Question

A tiny fifth singular value next to a null space. This is a one-case view of the repository's A tier; the complete profile remains the regression authority.

## Matrix or problem

A5 Hadamard mix with a tiny fifth value `2^-32` and a three-dimensional null space.
...
The smallest positive value competes directly with the tolerance and arithmetic error.
...
`mp_svd_tiers` with `tier="A"`, `case_id="A5-RANK5"`, and the public `svd` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

The case runner records reconstruction, orthogonality/unitarity, ordered nonnegative singular values, input identity, and precision roles. Inspect the ordered tail, rank at multiple tolerances, and reconstruction.
...
More precision can resolve the fifth value, but the reported numerical rank remains tolerance-dependent.
