...
**Corresponding example:** `examples/tiered/svd-tier-a/hadamard_rank4.m`

**Original tier/source:** The rank-four mode is defined by an explicit zero tail in the A5 manifest.

## Question

A deliberately rank-deficient dense matrix. This is a one-case view of the repository's A tier; the complete profile remains the regression authority.

## Matrix or problem

A5 Hadamard mix whose spectrum contains four positive values and a four-dimensional null space.
...
Exact rank deficiency separates structural zero directions from tiny positive values.
...
`mp_svd_tiers` with `tier="A"`, `case_id="A5-RANK4"`, and the public `svd` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

The case runner records reconstruction, orthogonality/unitarity, ordered nonnegative singular values, input identity, and precision roles. Inspect zero leakage, economy/full shapes, and `rank` under stated tolerances.
...
A failed inverse or a zero singular value is expected; it is not an arithmetic fallback case.
