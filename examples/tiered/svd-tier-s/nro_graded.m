% NRO graded block.
%
% Question: A block matrix with geometrically graded off-diagonal weights.
% Steps:
...
% Matching documentation: docs/examples/tiered/svd-tier-s/nro_graded.md
...
result = mp_svd_tiers ("smoke", struct ("tier", "S", ...
                                           "case_id", "S1-NRO-GRADED", ...
                                           "plot", false));
assert (result.scope_ok && strcmp (result.status, "PASS") ...
        && result.case_count == 1);
fprintf ("S1-NRO-GRADED: PASS; selected cases=%d; measured rows=%d\n", ...
         result.case_count, result.measured_svd_rows);
fprintf ("See docs/examples/tiered/svd-tier-s/nro_graded.md for interpretation.\n");
