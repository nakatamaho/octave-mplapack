...
**Corresponding example:** `examples/tiered/neig-tier-s/sim_two_jordan.m`

**Original tier/source:** The exact J layout and merged-cluster warning come from `docs/codex/neigt/CASES.md`.

## Question

Two nearby defective clusters and their merged-subspace warning. This is a one-case view of the repository's S tier; the complete profile remains the regression authority.

## Matrix or problem

S2 exact similarity with two size-2 Jordan blocks at 1 and `1+2^-12`.
...
Separated clusters can merge at a requested enclosure radius, so isolation and invariant-basis claims differ.
...
`mp_neig_tiers` with `tier="S"`, `case_id="SIM_TWO_JORDAN"`, and the public `eig` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

Inspect cluster separation, counted coverage, block factors, and whether the query is separated or merged.
...
Do not infer exact multiplicity from a disk count alone.
