% Run one original or explicitly scaled Forsythe eigensystem.
function row = nes_forsythe_run (parameters, work_bits, mode, model_q, reference, native)
  n = parameters.n;
  q = reference.reference_bits;
  if (native)
    built = nes_build ("forsythe", parameters, q);
    actual = built.native_A;
    source_bits = 53;
    input_precision = "native-binary64";
    backend = "native";
  else
    built = nes_build ("forsythe", parameters, work_bits);
    actual = built.A;
  ...
  end_try_catch
  mpbits (saved_bits);
  row = struct ("schema", "neig-v1", "family", "forsythe", ...
                "representation", parameters.representation, "n", n, ...
                "a", parameters.a, "s", NaN, "backend", backend, ...
                ...
                "native", native, "native_underflow", ternary (native, ...
                nes_build ("forsythe", parameters, q).native_underflow, false), ...
                ...
                "relative_mapping", [], "circle_mapping", [], ...
  ...
  circle_match = nes_match (metrics.values, reference.unit_roots, "circle", ...
                            mp ("1"), reference.radius);
  ...
  row.circle_error = circle_match.threshold;
  ...
  row.circle_mapping = circle_match.mapping;
