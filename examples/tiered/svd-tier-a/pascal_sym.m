...
%
% Question: A dense symmetric Pascal construction.
% Steps:
...
% Matching documentation: docs/examples/tiered/svd-tier-a/pascal_sym.md
...
result = mp_svd_tiers ("smoke", struct ("tier", "A", ...
                                           "case_id", "A1-PASCAL-SYM", ...
                                           "plot", false));
assert (result.scope_ok && strcmp (result.status, "PASS") ...
        && result.case_count == 1);
fprintf ("A1-PASCAL-SYM: PASS; selected cases=%d; measured rows=%d\n", ...
         result.case_count, result.measured_svd_rows);
fprintf ("See docs/examples/tiered/svd-tier-a/pascal_sym.md for interpretation.\n");
