...
%
% Question: The explicitly scaled normal companion control.
% Steps:
...
% Matching documentation: docs/examples/tiered/neig-tier-s/forsythe_scaled.md
...
result = mp_neig_tiers ("smoke", struct ("tier", "S", ...
                                           "case_id", "FORSYTHE_SCALED", ...
                                           "plot", false));
assert (result.ok && result.coverage.case_count == 1);
fprintf ("FORSYTHE_SCALED: PASS; selected cases=%d; measured rows=%d\n", ...
         result.coverage.case_count, result.coverage.measured_eig_rows);
fprintf ("See docs/examples/tiered/neig-tier-s/forsythe_scaled.md for interpretation.\n");
