% NRO two-level block.
%
% Question: A dense block matrix with two reciprocal singular-value levels.
% Steps:
...
% Matching documentation: docs/examples/tiered/svd-tier-s/nro_two.md
...
assert (result.scope_ok && strcmp (result.status, "PASS") ...
        && result.case_count == 1);
fprintf ("S1-NRO-TWO: PASS; selected cases=%d; measured rows=%d\n", ...
         result.case_count, result.measured_svd_rows);
fprintf ("See docs/examples/tiered/svd-tier-s/nro_two.md for interpretation.\n");
