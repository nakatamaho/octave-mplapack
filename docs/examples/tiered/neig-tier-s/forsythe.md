# Forsythe cyclic perturbation

**Corresponding example:** `examples/tiered/neig-tier-s/forsythe.m`

**Original tier/source:** The split/scaled/zero formulas are in `docs/codex/neigt/CASES.md` Section 4.

## Question

The tiny-corner cyclic perturbation of a Jordan block. This is a one-case view of the repository's S tier; the complete profile remains the regression authority.

## Matrix or problem

S4 `I+N` with the `(n,1)` entry `2^(-a*n)`, n=8 and a=4.
...
A tiny cyclic closure moves a Jordan spectrum onto a small circle and creates scale-sensitive eigenvectors.
...
`mp_neig_tiers` with `tier="S"`, `case_id="FORSYTHE"`, and the public `eig` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

Inspect `(lambda-1)/r` against MP unit-circle roots, residuals, and native-underflow metadata.
...
The zero-limit and scaled controls are different inputs and must remain labeled.
