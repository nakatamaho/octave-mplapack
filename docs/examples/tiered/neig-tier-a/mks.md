...
**Corresponding example:** `examples/tiered/neig-tier-a/mks.m`

**Original tier/source:** The rank-one perturbation definition is in the NEIG case ledger.

## Question

A nilpotent power plus a rank-one perturbation. This is a one-case view of the repository's A tier; the complete profile remains the regression authority.

## Matrix or problem

A2 MKS `N^m + delta*ones(n)*ones(n)'` with n=12, m=3, delta=1/8.
...
A low-rank perturbation of a nilpotent power can produce sensitive, nonnormal eigenstructure.
...
`mp_neig_tiers` with `tier="A"`, `case_id="MKS"`, and the public `eig` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

Inspect eigenvalue matching, rank-one scale, residuals, and condition estimates.
...
Do not reduce this to a diagonal or symmetric proxy.
