...
The repository includes a runnable difficult-eigensystem experiment based on the existing public `mp`, `mpbits`, `eig`, `norm`, and MP inspection functions. It is a QA/example harness, not an additional package-level API. Run it from a checkout with:

    tools/run-nonsymmetric-eig-suite.sh smoke
    tools/run-nonsymmetric-eig-suite.sh demo

The smoke profile has 30 rows and the demo profile has 40 rows. Each row is one of five representations (Hadamard-similar, Frank, companion, Forsythe original, or explicitly scaled Forsythe), one of `balance` and `nobalance`, and one explicit native-binary64 or MP work precision. Native rows are rounded-input controls; they are never substituted for MP rows.

The harness constructs MP input matrices at the requested work precision, restores the ambient `mpbits ()` default, and evaluates the actual solver outputs at a separate high precision. It checks `A*V = V*D`, `W'*A = D*W'`, column residuals, one-to-one eigenvalue matches, and left/right eigenvector condition estimates. It does not assume solver output ordering or vector normalization. For a high-quality example, the Hadamard-similar model has exact integer eigenvalues but can be highly nonnormal, while the Forsythe pair separates a small coupling from a native underflow control.
