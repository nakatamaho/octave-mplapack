...
Smoke: n=8. Demo: n=24. Q is lower triangular with binomial integer entries.

$$
Q_{ij}=binom(i-1,j-1)  if j<=i,    Q_{ij}=0 otherwise.
$$
Q is unit lower triangular, so det(Q)=1. Its entries are generated by Pascal addition, not by native factorial arithmetic.
