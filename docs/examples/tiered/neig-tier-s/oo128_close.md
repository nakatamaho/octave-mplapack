...
**Corresponding example:** `examples/tiered/neig-tier-s/oo128_close.m`

**Original tier/source:** The authoritative provenance is the manifest and `GENERATOR-OO.md`, including its fixed generation precision.

## Question

The 128-bit close-spectrum paired-block case. This is a one-case view of the repository's S tier; the complete profile remains the regression authority.

## Matrix or problem

S1 Ozaki--Ogita close-spectrum form generated at 128 bits.
...
Close realized eigenvalues make forward eigenvalue error and eigenvector error different questions.
...
`mp_neig_tiers` with `tier="S"`, `case_id="OO128_CLOSE"`, and the public `eig` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

Inspect eigenvalue separations, matched forward errors, both residual identities, and exactness status.
...
The close spectrum is a property of the realized fixture, not an ideal list invented by the example.
