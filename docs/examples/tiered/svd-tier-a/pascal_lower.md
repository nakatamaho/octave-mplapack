...
**Corresponding example:** `examples/tiered/svd-tier-a/pascal_lower.m`

**Original tier/source:** The Pascal definitions and provenance are in `docs/codex/svt/CASES.md` Section A1.

## Question

A triangular totally-nonnegative Pascal matrix. This is a one-case view of the repository's A tier; the complete profile remains the regression authority.

## Matrix or problem

A1 lower Pascal matrix of order 8, generated by exact binomial recurrence.
...
Triangular total nonnegativity and rapidly varying scales make the singular spectrum broad.
...
`mp_svd_tiers` with `tier="A"`, `case_id="A1-PASCAL-LOWER"`, and the public `svd` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

The case runner records reconstruction, orthogonality/unitarity, ordered nonnegative singular values, input identity, and precision roles. Inspect reconstruction, ordered nonnegative values, and full/economy shape agreement.
...
Total nonnegativity is not a promise of a well-conditioned SVD.
