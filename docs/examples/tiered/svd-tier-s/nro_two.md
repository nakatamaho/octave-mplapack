...
**Corresponding example:** `examples/tiered/svd-tier-s/nro_two.m`

**Original tier/source:** `docs/codex/svt/CASES.md` Section S1 and `docs/codex/svt/SOURCES.md`; the short example selects the exact manifest case.

## Question

A dense block matrix with two reciprocal singular-value levels. This is a one-case view of the repository's S tier; the complete profile remains the regression authority.

## Matrix or problem

S1 NRO block with m=4 and all off-diagonal weights `2^12`, giving two reciprocal singular-value levels.
...
The dense block is nonnormal and has a large/small reciprocal pair structure despite simple integer entries.
...
`mp_svd_tiers` with `tier="S"`, `case_id="S1-NRO-TWO"`, and the public `svd` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

The case runner records reconstruction, orthogonality/unitarity, ordered nonnegative singular values, input identity, and precision roles. Inspect `A-U*S*V'`, U/V orthogonality, descending nonnegative `diag(S)`, and the stored work precision.
...
MP arithmetic reduces rounding error; it cannot improve the conditioning of the NRO block itself.
