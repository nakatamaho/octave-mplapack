...
**Corresponding example:** `examples/tiered/svd-tier-a/pascal_sym.m`

**Original tier/source:** The symmetric recurrence is a separate manifest case in the A1 family.

## Question

A dense symmetric Pascal construction. This is a one-case view of the repository's A tier; the complete profile remains the regression authority.

## Matrix or problem

A1 symmetric Pascal construction of order 8.
...
Symmetry provides an orthogonal control while the entries still grow combinatorially.
...
`mp_svd_tiers` with `tier="A"`, `case_id="A1-PASCAL-SYM"`, and the public `svd` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

The case runner records reconstruction, orthogonality/unitarity, ordered nonnegative singular values, input identity, and precision roles. Inspect orthogonality and compare spectral quantities with the lower case, not individual vectors.
...
A symmetric input changes the backend path; it is not the same coordinate problem as the lower triangle.
