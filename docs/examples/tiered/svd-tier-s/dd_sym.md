...
**Corresponding example:** `examples/tiered/svd-tier-s/dd_sym.m`

**Original tier/source:** The selected path formula and below-guard policy are in `docs/codex/svt/CASES.md` Section S4.

## Question

A nearly singular symmetric tridiagonal path. This is a one-case view of the repository's S tier; the complete profile remains the regression authority.

## Matrix or problem

S4 symmetric diagonally dominant path with n=8, tau=`2^-32`, rho=1.
...
The smallest singular value is close to the diagonal-dominance margin, so cancellation and rank thresholds matter.
...
`mp_svd_tiers` with `tier="S"`, `case_id="S4-DD-SYM"`, and the public `svd` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

The case runner records reconstruction, orthogonality/unitarity, ordered nonnegative singular values, input identity, and precision roles. Inspect reconstruction, orthogonality, ordered values, and compare `rank(A,tol)` with the smallest value.
...
MP helps preserve tau; it cannot make a near-singular problem well-conditioned.
