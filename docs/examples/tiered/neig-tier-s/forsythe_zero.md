# Forsythe zero-limit Jordan block

**Corresponding example:** `examples/tiered/neig-tier-s/forsythe_zero.m`

**Original tier/source:** The zero case is explicitly required by `docs/codex/neigt/CASES.md`; it is not a failed small-perturbation solve.

## Question

The genuine zero-coupling Jordan limit. This is a one-case view of the repository's S tier; the complete profile remains the regression authority.

## Matrix or problem

S4 exact `I+N`, the genuine zero-coupling limit and a single size-n Jordan block.
...
Every eigenvalue is repeated and the matrix has no full eigenbasis.
...
`mp_neig_tiers` with `tier="S"`, `case_id="FORSYTHE_ZERO"`, and the public `eig` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

Inspect the defective cluster and block-SCHUR/invariant-subspace certificate; residual alone is insufficient.
...
Never apply a simple-root condition target to this case.
