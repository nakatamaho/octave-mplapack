% Run one Frank eigensystem using the actual input representation.
function row = nes_frank_run (parameters, work_bits, mode, model_q, reference, native)
  n = parameters.n;
  q = reference.reference_bits;
  if (native)
    built = nes_build ("frank", parameters, q);
    actual = built.native_A;
    source_bits = 53;
    input_precision = "native-binary64";
    backend = "native";
  else
    built = nes_build ("frank", parameters, work_bits);
    actual = built.A;
  ...
  end_try_catch
  mpbits (saved_bits);
  row = struct ("schema", "neig-v1", "family", "frank", ...
                "representation", "frank", "n", n, "s", NaN, ...
                "backend", backend, "work_bits", work_bits, ...
                "evaluation_bits", q, "reference_bits", reference.reference_bits, ...
                "low_reference_bits", reference.low_reference_bits, ...
                "reference_agreement", reference.reference_agreement, ...
                "reference_agreement_limit", reference.reference_agreement_limit, ...
                "mode", mode, "input_precision", input_precision, ...
                "native", native, "solver_status", solver_status, ...
                "solver_error", solver_error, "solver_time", solver_time, ...
                "input_error", input_error, "input_error_relative", ...
                input_error_relative, "accuracy_status", "not_targeted", ...
                "reference_status", reference.reference_status, ...
                "condition_status", "unresolved", "max_imaginary", mp ("NaN"), ...
                "absolute_error", mp ("NaN"), "relative_error", mp ("NaN"), ...
                "right_residual", mp ("NaN"), "left_residual", mp ("NaN"), ...
                "right_column_residual", mp ("NaN"), "left_column_residual", ...
                mp ("NaN"), "condition_estimates", mp ("NaN"), ...
                "vector_condition", mp ("NaN"), "condition_disagreement", ...
                mp ("NaN"), "absolute_mapping", [], "relative_mapping", [], ...
                "computed_values", [], "reference_values", reference.eigenvalues);
