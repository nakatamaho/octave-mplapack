...
%
% Question: A complex phase Lauchli control.
% Steps:
...
% Matching documentation: docs/examples/tiered/svd-tier-a/lauchli_complex.md
...
result = mp_svd_tiers ("demo", struct ("tier", "A", ...
                                           "case_id", "A4-LAU-COMPLEX", ...
                                           "plot", false));
assert (result.scope_ok && strcmp (result.status, "PASS") ...
        && result.case_count == 1);
fprintf ("A4-LAU-COMPLEX: PASS; selected cases=%d; measured rows=%d\n", ...
         result.case_count, result.measured_svd_rows);
fprintf ("See docs/examples/tiered/svd-tier-a/lauchli_complex.md for interpretation.\n");
