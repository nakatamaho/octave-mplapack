% Unsigned Lah matrix.
%
% Question: A rapidly growing unit lower-triangular combinatorial matrix.
% Steps:
...
% Matching documentation: docs/examples/tiered/svd-tier-s/lah.md
...
assert (result.scope_ok && strcmp (result.status, "PASS") ...
        && result.case_count == 1);
fprintf ("S3-LAH: PASS; selected cases=%d; measured rows=%d\n", ...
         result.case_count, result.measured_svd_rows);
fprintf ("See docs/examples/tiered/svd-tier-s/lah.md for interpretation.\n");
