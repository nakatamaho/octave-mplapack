...
Current substage: `D00-T` release-tag gate

Last completed substage: `D00-R` reproducibility

Overall result: `IN PROGRESS`

The source freeze identities and all pre-tag artifacts are fixed. Release tags
are deliberately created only after the pre-tag gates below have been
recorded as PASS.
...
| D00-T tags | PENDING | Create and verify `v3.0.1` and `v0.2.0` only after this status/report commit |
| D00-H handoff | PENDING | Complete after remote tag verification and final manifest/report update |
...
GMPFRXX_ARCHIVE=gmpfrxx_mkII.1.4.1-A.tar.xz
...
MPLAPACK_RELEASE_TAG=pending D00-T (v3.0.1)
...
OCTAVE_MPLAPACK_RELEASE_TAG=pending D00-T (v0.2.0)
