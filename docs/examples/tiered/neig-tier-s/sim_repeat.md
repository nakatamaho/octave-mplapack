...
**Corresponding example:** `examples/tiered/neig-tier-s/sim_repeat.m`

**Original tier/source:** The case follows the exact similarity definitions in `docs/codex/neigt/CASES.md`.

## Question

A repeated eigenvalue with a two-dimensional eigenspace. This is a one-case view of the repository's S tier; the complete profile remains the regression authority.

## Matrix or problem

S2 exact similarity with a semisimple repeated eigenvalue at the leading 2-by-2 block.
...
The eigenspace is two-dimensional; individual eigenvectors are not canonical.
...
`mp_neig_tiers` with `tier="S"`, `case_id="SIM_REPEAT"`, and the public `eig` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

Inspect the matched repeated cluster and an invariant-subspace or projector diagnostic, not vector-by-vector equality.
...
A repeated root is not a solver failure and must not be counted as a simple isolated root.
