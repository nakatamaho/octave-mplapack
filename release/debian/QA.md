...
| P04 binary build | PARTIAL | clean resolute sbuild build succeeds with injected MPC 1.4.1; lifecycle pending |
...
| piuparts | PENDING | installed; package lifecycle run remains |
...
The next local action is to close P02 provider/package policy and run the
installed piuparts/autopkgtest/reprotest lifecycle checks without changing the
released upstream numerical sources. The clean build already uses an Ubuntu
26.04/resolute amd64 schroot; no Debian/sid or publication identity is implied.
