# NRO scale-up control

**Corresponding example:** `examples/tiered/svd-tier-s/nro_scale_up.m`

**Original tier/source:** The scale control is a demo-only manifest variant of the NRO source family.

## Question

A global dyadic scale-up of the NRO block. This is a one-case view of the repository's S tier; the complete profile remains the regression authority.

## Matrix or problem

S1 NRO two-level block with an exact global dyadic scale-up (demo control).
...
Scaling tests exponent range and relative diagnostics without changing singular-vector geometry.
...
`mp_svd_tiers` with `tier="S"`, `case_id="S1-NRO-SCALE-UP"`, and the public `svd` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

The case runner records reconstruction, orthogonality/unitarity, ordered nonnegative singular values, input identity, and precision roles. Inspect that singular values scale while normalized factors and residuals remain comparable.
...
Absolute tolerances must be scale-aware; never infer a regression from a raw unnormalized residual alone.
