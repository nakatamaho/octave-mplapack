...
%
% Question: The genuine zero-coupling Jordan limit.
% Steps:
...
% Matching documentation: docs/examples/tiered/neig-tier-s/forsythe_zero.md
...
result = mp_neig_tiers ("smoke", struct ("tier", "S", ...
                                           "case_id", "FORSYTHE_ZERO", ...
                                           "plot", false));
assert (result.ok && result.coverage.case_count == 1);
fprintf ("FORSYTHE_ZERO: PASS; selected cases=%d; measured rows=%d\n", ...
         result.coverage.case_count, result.coverage.measured_eig_rows);
fprintf ("See docs/examples/tiered/neig-tier-s/forsythe_zero.md for interpretation.\n");
