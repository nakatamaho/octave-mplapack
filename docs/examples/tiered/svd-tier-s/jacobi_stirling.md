...
**Corresponding example:** `examples/tiered/svd-tier-s/jacobi_stirling.m`

**Original tier/source:** The recurrence and leading rows are in `docs/codex/svt/CASES.md` Section S2.

## Question

A unit lower-triangular combinatorial matrix. This is a one-case view of the repository's S tier; the complete profile remains the regression authority.

## Matrix or problem

S2 Jacobi--Stirling second-kind matrix, n=8 and z=1, generated by its integer recurrence.
...
Combinatorial growth makes the lower-triangular matrix full rank but badly scaled.
...
`mp_svd_tiers` with `tier="S"`, `case_id="S2-JS"`, and the public `svd` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

The case runner records reconstruction, orthogonality/unitarity, ordered nonnegative singular values, input identity, and precision roles. Inspect reconstruction, ordered nonnegative values, economy shapes, and `rank`/tolerance diagnostics.
...
Unit diagonal proves full rank, not accurate small singular values at insufficient precision.
