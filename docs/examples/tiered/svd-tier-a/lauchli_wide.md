...
**Corresponding example:** `examples/tiered/svd-tier-a/lauchli_wide.m`

**Original tier/source:** The wide shape is a distinct manifest case with the same small scale.

## Question

A wide transpose-oriented Lauchli matrix. This is a one-case view of the repository's A tier; the complete profile remains the regression authority.

## Matrix or problem

A4 wide Lauchli matrix, the transpose-oriented representation of the tall control.
...
Wide matrices expose the complementary economy dimensions and null-space behavior.
...
`mp_svd_tiers` with `tier="A"`, `case_id="A4-LAU-WIDE"`, and the public `svd` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

The case runner records reconstruction, orthogonality/unitarity, ordered nonnegative singular values, input identity, and precision roles. Inspect `size(U),size(S),size(V)`, reconstruction, and zero leakage.
...
Do not assume tall and wide factor arrays have interchangeable shapes.
