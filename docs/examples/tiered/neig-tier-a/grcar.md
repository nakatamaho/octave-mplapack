# Grcar nonnormal band matrix

**Corresponding example:** `examples/tiered/neig-tier-a/grcar.m`

**Original tier/source:** The case parameters are frozen in `docs/codex/neigt/cases.json`; the public manual also uses Grcar as a basic example.

## Question

A dense-output nonsymmetric Grcar band matrix. This is a one-case view of the repository's A tier; the complete profile remains the regression authority.

## Matrix or problem

A2 Grcar band matrix with n=12 and upper bandwidth 3.
...
Nonnormality and complex conjugate eigenvalues make ordering and phase comparisons unreliable.
...
`mp_neig_tiers` with `tier="A"`, `case_id="GRCAR"`, and the public `eig` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

Inspect both left/right residuals, conjugate matching, and balance versus no-balance.
...
More bits improve arithmetic residuals, not the pseudospectral sensitivity of Grcar.
