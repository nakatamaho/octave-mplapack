...
**Corresponding example:** `examples/tiered/svd-tier-a/bidiag_mixed.m`

**Original tier/source:** The manifest distinguishes the mixed representation; its transform is retained by the full runner.

## Question

A sign-mixed bidiagonal control with the same singular geometry. This is a one-case view of the repository's A tier; the complete profile remains the regression authority.

## Matrix or problem

A3 mixed bidiagonal representation of the same source family.
...
Mixing tests input identity and factor robustness without changing the public SVD call.
...
`mp_svd_tiers` with `tier="A"`, `case_id="A3-BDI-MIXED"`, and the public `svd` path. The operation restores the caller's ambient `mpbits()` default after the run. Native controls, work-precision results, reference values, and diagnostics are separate records.

## What to inspect

The case runner records reconstruction, orthogonality/unitarity, ordered nonnegative singular values, input identity, and precision roles. Inspect reconstruction and compare singular values, while treating factor signs/phases as noncanonical.
...
Do not claim equality of factor entries across an orthogonal mix.
