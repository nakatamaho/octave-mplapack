...
%
% Question: A dense block matrix with extreme and unit singular groups.
% Steps:
...
% Matching documentation: docs/examples/tiered/svd-tier-s/nro_three.md
...
result = mp_svd_tiers ("smoke", struct ("tier", "S", ...
                                           "case_id", "S1-NRO-THREE", ...
                                           "plot", false));
assert (result.scope_ok && strcmp (result.status, "PASS") ...
        && result.case_count == 1);
fprintf ("S1-NRO-THREE: PASS; selected cases=%d; measured rows=%d\n", ...
         result.case_count, result.measured_svd_rows);
fprintf ("See docs/examples/tiered/svd-tier-s/nro_three.md for interpretation.\n");
