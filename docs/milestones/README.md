# Milestone index

Work proceeds one milestone at a time. A milestone is complete only when its
documented gate passes.

| Milestone | Purpose | Status |
|-----------|---------|--------|
| M00 | Repository bootstrap | Passed |
| M01 | Native build probe | Passed |
| M02 | Internal native MPFR scalar storage | Passed |
| M03 | Public scalar constructor and wrapper | Passed |
| M04 | Precision API | Passed |
| M05 | Conversion/display | Passed |
| M06 | Scalar element-wise arithmetic | Passed |
| M07 | Native dense matrix storage | Passed |
| M08 | Matrix multiplication | Passed |
| M09 | Linear solve | Passed |
| M10 | Dense matrix inspection | Passed |
| M11 | Dense element-wise arithmetic | Passed |
| M12 | Dense transpose and reshape | Passed |
| M13 | Dense horizontal and vertical concatenation | Passed |
| M14 | Dense indexed assignment with value semantics | Passed |
| M15 | Full-rank rectangular dense solve | Passed |
| M16 | Rank-deficient rectangular minimum-norm solve | Passed |
| M17 | Dense real Cholesky factorization | Passed |
| M18 | Dense real non-pivoted QR factorization | Passed |
| M19 | Dense real pivoted QR factorization | Passed |
| M20 | Complex architecture audit and design freeze | Passed (design-only) |
| M21 | Dense real LU factorization | Passed |
| M22 | Real API and release closure for v0.1 | In progress |
| P00 | Packaging design | Planned |
| P01 | Debian source package | Planned |
| P02 | Local Debian build | Planned |
| P03 | PPA staging | Planned |
| P04 | PPA autopkgtest | Planned |
| P05 | PPA stable | Planned |
| P06 | Ubuntu series/architecture matrix | Planned |
