...
%
% Question: A nearly singular nonsymmetric tridiagonal path.
% Steps:
...
% Matching documentation: docs/examples/tiered/svd-tier-s/dd_nonsym.md
...
result = mp_svd_tiers ("smoke", struct ("tier", "S", ...
                                           "case_id", "S4-DD-NONSYM", ...
                                           "plot", false));
assert (result.scope_ok && strcmp (result.status, "PASS") ...
        && result.case_count == 1);
fprintf ("S4-DD-NONSYM: PASS; selected cases=%d; measured rows=%d\n", ...
         result.case_count, result.measured_svd_rows);
fprintf ("See docs/examples/tiered/svd-tier-s/dd_nonsym.md for interpretation.\n");
