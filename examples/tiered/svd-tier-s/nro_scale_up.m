...
%
% Question: A global dyadic scale-up of the NRO block.
% Steps:
...
% Matching documentation: docs/examples/tiered/svd-tier-s/nro_scale_up.md
...
result = mp_svd_tiers ("demo", struct ("tier", "S", ...
                                           "case_id", "S1-NRO-SCALE-UP", ...
                                           "plot", false));
assert (result.scope_ok && strcmp (result.status, "PASS") ...
        && result.case_count == 1);
fprintf ("S1-NRO-SCALE-UP: PASS; selected cases=%d; measured rows=%d\n", ...
         result.case_count, result.measured_svd_rows);
fprintf ("See docs/examples/tiered/svd-tier-s/nro_scale_up.md for interpretation.\n");
